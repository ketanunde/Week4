...
# Now we remove all other data
# first we extract column names
col_names <- colnames(file_data)
col_names

# build vector that contains TRUE for each element within it.
# The logical() creates logical vector o specified length
...
# we want to remove columns word "age" in them and 
# except "age18to44" and "Age_45_above".
?grepl
 # we are using Sapply() and grepl() function to search for "Age"
...
# This causes an issue - i want to keep "Age_18to44" and "Age_45_above"
# Do it manually
variables_list["Age_18to44"] <- TRUE
variables_list["Age_45_above"] <- TRUE
variables_list["AdultMeanAge"] <- TRUE

# Finally i want to remove other vars i combined earlier
...
# This vector variable list what we do lke to have
# so i use it for new data frame
adjusted_data <- file_data[, variables_list]
nrow(adjusted_data)

# write Adjusted file in csv for future
write.csv(adjusted_data, file = "data_brexit_referendum_adjusted_data.csv")
